...
public:
    ...
    size_t GetUid();
    int InitialMyInfo(transfOnPer &m,transfPartner &s,channel *p);
...
private:
    ...
    */
    ...
    int GenericSend(uint32_t id, const size_t &dest,const char*buf,int size,size_t from);
private:
    size_t uid_; //使用者的uid
    /*内部需要维护一个朋友队列吗? */
...
public:
    ...
    std::shared_ptr<user> FindByUid(size_t&uid);
