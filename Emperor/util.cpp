...
{
    uint64_t t=crc64(0,(unsigned char*)&m,STRUCTONPERLEN-8);

    return (std::memcmp(&t,&m.crc32,8)==0)?UTILNET_SUCCESS :UTILNET_ERROR;
}

int genCrcPayload(transfOnPer &m)
{
    m.crc32=crc64(0,(unsigned char*)&m,STRUCTONPERLEN-8);
    return UTILNET_SUCCESS;
}
...
Status::Status(Code code, const char* msg, const char* msg2) {
  assert(code != kOk);
  const uint32_t len1 = strlen(msg);
  const uint32_t len2 = strlen(msg2);
  const uint32_t size = len1 + (len2 ? (2 + len2) : 0);
  char* result = new char[size + 5];
  memcpy(result, &size, sizeof(size));
  result[4] = static_cast<char>(code);
  memcpy(result + 5, msg, len1);
  if (len2) {
    result[5 + len1] = ':';
    result[6 + len1] = ' ';
    memcpy(result + 7 + len1, msg2, len2);
  }
  state_ = result;
}
